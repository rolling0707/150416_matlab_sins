...
pos0 = [40.0678531*glv.deg; 116.247326*glv.deg; 47.92];     %初始位置   
vn0 = [0; 0; 0];                                            %初始速度，此时为静止态
att0 = [0.0040; -0.0048; -2.9976];                          %采用UKF滤波对准最后阶段的姿态值
cnb0 = a2cnb(att0);
qnb0 = m2qnb(cnb0);                                         %姿态值变化为初始四元数
...
Zk_kalman = zeros(lenp, 6);                                 %观测量存储，即速度和位置误差
navigation = zeros(len, 9);                                 %导航参数存储
kv = 2035;                                                  %与此计算中的IMU数据对齐
kk = 1;
kerr = 1;



for k = 454321:n:716560                               %10.5公里行车距离，20分钟
