...
%    wm = sum(wm,2);
%    vm = sum(vm,2);
%    Ft = JacobianF(qnb, vn, pos, vm./Ts, Xk);
%    [Fikk_1, Qk] = kfdis(Ft, Qt, Ts, 4);
%    fX = efff(qnb, vn, pos, vm./Ts, Xk);
%    for j = k:1:k1
...
save alignment_ekf navigation
save Xkk_ekf Xkk_ekf
save Zk_ekf Zk_ekf
...
% 
% figure,             %画出俯仰误差角曲线
% plot(Xkk_ekf(1:kerr-1,1)/glv.deg, '-b'), grid on
% xlabel('\itt/s'); ylabel('\it\theta\rm / \circ'); grid on
% 
% figure,             %画出俯横滚差角曲线
% plot(Xkk_ekf(1:kerr-1,2)/glv.deg, '-b'), grid on
% xlabel('\itt/s'); ylabel('\it\gamma\rm / \circ');
% 
% figure,             %画出俯方位差角曲线
% plot(Xkk_ekf(1:kerr-1,3)/glv.deg, '-b'), grid on
% xlabel('\itt/s'); ylabel('\it\psi\rm / \circ');
% 
% figure,             %画出东北向速度误差
% subplot(2,1,1), plot(Zk_ekf(1:kerr-1,1)), grid on
% xlabel('\itt/s'); ylabel('\it\delta Ve\rm /m/s'); 
% subplot(2,1,2), plot(Zk_ekf(1:kerr-1,2)), grid on
% xlabel('\itt/s'); ylabel('\it\delta Vn\rm /m/s'); 
% 
% figure,             %画出纬度、经度误差
% subplot(2,1,1), plot(Zk_ekf(1:kerr-1,4)*glv.Re), grid on
% xlabel('\itt/s'); ylabel('\it\delta Latitude\rm /m'); 
% subplot(2,1,2), plot(Zk_ekf(1:kerr-1,5)*glv.Re), grid on
